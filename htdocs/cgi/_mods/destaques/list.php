...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_destaque']?>" id="label<?=$item['id_destaque']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_destaque']?>" value="<?=$item['id_destaque']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_destaque']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><span style="display:none"><?=$item['data']?></span><?=ajustadata($item['data'],"site")?></td>
            <td><?=$item['titulo']?></td>
            <td><span style="display:none"><?=$item['descricao']?></span><?=substr($item['descricao'],0,20)?>...</td>
		</tr>
<?php
	}
?>
