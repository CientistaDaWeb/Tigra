...
<?php
$busca = $con_cliente->executa("SELECT * FROM tags");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_tag']?>" id="label<?=$item['id_tag']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_tag']?>" value="<?=$item['id_tag']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_tag']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['tag']?></td>
		</tr>
<?php
	}
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhuma palavra chave.</span></div>
<?php
}
?>
</form>
