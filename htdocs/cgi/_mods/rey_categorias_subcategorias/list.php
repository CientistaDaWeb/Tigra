...
<?php
//$busca = $con_cliente->executa('SELECT * FROM categorias_subcategorias');
...
$SQL = '
    ...
    ON cs.id_produtos_categorias = pc.id_produtos_categoria
';
$busca = $con_cliente->executa($SQL);
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_categorias_subcategoria']?>" id="label<?=$item['id_categorias_subcategoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_categorias_subcategoria']?>" value="<?=$item['id_categorias_subcategoria']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_categorias_subcategoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhuma subcategorias de produtos.</span></div>
<?php
}
?>
</form>
