...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_infraestrutura']?>" id="label<?=$item['id_infraestrutura']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_infraestrutura']?>" value="<?=$item['id_infraestrutura']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_infraestrutura']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
