...
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" id="form_deletar" onsubmit="return valida_deletar();">
    ...
    <?php
    $query = 'SELECT c.id_curso, c.curso, cc.categoria FROM cursos c, cursos_categorias cc WHERE c.id_cursos_categoria = cc.id_cursos_categoria';
    ...
    <table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
        ...
        <tbody>
                ...
                while($item = mysqli_fetch_assoc($busca)) {
                    ?>
            <tr>
                <td class="campo_checkbox"><label for="checkbox<?=$item['id_curso']?>" id="label<?=$item['id_curso']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_curso']?>" value="<?=$item['id_curso']?>" /></td>
                <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_curso']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
                <td><?=$item['curso']?></td>
        ...
        ?>
    <div class="vazio">Não foi encontrado nenhum curso.</div>
        <?php
    }
    ?>
</form>
