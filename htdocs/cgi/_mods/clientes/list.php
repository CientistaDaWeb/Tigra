...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_cliente']?>" id="label<?=$item['id_cliente']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_cliente']?>" value="<?=$item['id_cliente']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_cliente']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['cliente']?></td>
		</tr>
<?php
	}
