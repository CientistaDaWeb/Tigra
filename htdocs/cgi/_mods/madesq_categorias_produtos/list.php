...
<?php
$busca = $con_cliente->executa('SELECT * FROM produtos_categorias');
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_produtos_categoria']?>" id="label<?=$item['id_produtos_categoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_produtos_categoria']?>" value="<?=$item['id_produtos_categoria']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_produtos_categoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['categoria']?></td>
            <?
            if($item['main_cat'] == 1) $main_cat = 'Móveis';
            else if($item['main_cat'] == 2) $main_cat = 'Esquadrias';
            else if($item['main_cat'] == 3) $main_cat = 'Fechaduras';
            else $main_cat = 'Puxadores';
            ?>
            <td><?=$main_cat?></td>
		</tr>
<?php
