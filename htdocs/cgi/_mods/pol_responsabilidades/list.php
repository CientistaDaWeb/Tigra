...
<?php
$busca = $con_cliente->executa('SELECT * FROM responsabilidades');
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_responsabilidade']?>" id="label<?=$item['id_responsabilidade']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_responsabilidade']?>" value="<?=$item['id_responsabilidade']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_responsabilidade']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['titulo']?></td>
		</tr>
<?php
	}
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrado nenhuma matéria de Responsabilidade Social.</span></div>
<?php
}
?>
</form>
