...
<?php
$busca = $con_cliente->executa("SELECT * FROM familiares");
...
<?php
	while($item = mysqli_fetch_assoc($busca)){	
		$pai = $con_cliente->executa("SELECT nome from familiares WHERE id_familiare = $item[fk_pai]");
		$pai_nome = '';
		if($pai && mysqli_num_rows($pai)>0){
			$pai = mysqli_fetch_assoc($pai)	;
			$pai_nome = $pai['nome'];
		}
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_familiare']?>" id="label<?=$item['id_familiare']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_familiare']?>" value="<?=$item['id_familiare']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_familiare']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
            <td><?=$pai_nome?></td>
		</tr>
<?php
