...
<?php
$busca = $con_cliente->executa("SELECT id_foto, legenda FROM fotos");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><input type="checkbox" name="del_item[]" id="checkbox<?=$item['id_foto']?>" value="<?=$item['id_foto']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_foto']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['legenda']?></td>
		</tr>
<?php
	}
