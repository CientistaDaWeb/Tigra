...
<?php
$busca = $con_cliente->executa('SELECT * FROM revestimentos_categorias');
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_revestimentos_categoria']?>" id="label<?=$item['id_revestimentos_categoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_revestimentos_categoria']?>" value="<?=$item['id_revestimentos_categoria']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_revestimentos_categoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrada nenhuma categoria de revestimentos.</span></div>
<?php
}
?>
</form>
