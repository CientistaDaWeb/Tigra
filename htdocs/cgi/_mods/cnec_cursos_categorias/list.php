...
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" id="form_deletar" onsubmit="return valida_deletar();">
    ...
    <?php
    $query = 'SELECT cc.id_cursos_categoria, cc.categoria, s.setor FROM cursos_categorias cc, setors s WHERE cc.id_setor = s.id_setor';
    ...
    <table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
        ...
        <tbody>
                ...
                while($item = mysqli_fetch_assoc($busca)) {
                    ?>
            <tr>
                <td class="campo_checkbox"><label for="checkbox<?=$item['id_cursos_categoria']?>" id="label<?=$item['id_cursos_categoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_cursos_categoria']?>" value="<?=$item['id_cursos_categoria']?>" /></td>
                <td class="campo_editar"><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_cursos_categoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
        ...
        ?>
    <div><span class="vazio">Não foi encontrada nenhuma categoria de cursos.</span></div>
        <?php
    }
    ?>
</form>
