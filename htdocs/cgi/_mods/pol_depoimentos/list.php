...
<?php
$busca = $con_cliente->executa("SELECT * FROM depoimentos");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_depoimento']?>" id="label<?=$item['id_depoimento']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_depoimento']?>" value="<?=$item['id_depoimento']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_depoimento']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
            <td><?=$item['cargo']?></td>
		</tr>
<?php
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrado nenhum depoimento.</span></div>
<?php
}
?>
</form>
