...
<?php
$busca = $con_cliente->executa("SELECT * FROM clippings");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_clipping']?>" id="label<?=$item['id_clipping']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_clipping']?>" value="<?=$item['id_clipping']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_clipping']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrado nenhum clipping.</span></div>
<?php
}
?>
</form>
