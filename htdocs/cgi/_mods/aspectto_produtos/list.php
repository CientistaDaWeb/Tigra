<style>
    .vSite{color:#2F2F2F}
    .vRestrito{color:#DF373E; font-weight:bold;}
</style>
...
<div id="tabs">
    ...
    <form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" id="form_deletar" onsubmit="return valida_deletar();">
        ...
        <div id="tab-<?=$categoria['slug']?>">
            ...
            $query = 'SELECT p.id_produto, p.nome, p.visibilidade, pc.categoria FROM produtos p INNER JOIN produtos_categorias pc ON p.id_produtos_categoria = pc.id_produtos_categoria WHERE pc.slug="'.$categoria['slug'].'" ORDER BY p.nome';
            ...
            <table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr class="<?=$zebrado[$i%2]?>">
                        ...
                        <td><?=$item['nome']?></td>
                        <td><?=$visibilidade[$item['visibilidade']]?></td>
                    </tr>
                    <?php
                }
                ?>
