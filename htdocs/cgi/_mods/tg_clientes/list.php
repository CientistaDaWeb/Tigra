...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_tg_cliente']?>" id="label<?=$item['id_tg_cliente']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_tg_cliente']?>" value="<?=$item['id_tg_cliente']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_tg_cliente']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
            <td><span style="display:none"><?=$item['nome']?></span><a href="http://www.weentigra.com.br/login/<?=cripfy($item['id_tg_cliente'],'id_tg_cliente')?>">Ir para o site</a></td>
		</tr>
<?php
	}
?>
