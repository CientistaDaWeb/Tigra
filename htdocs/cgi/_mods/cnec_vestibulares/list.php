...
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" id="form_deletar" onsubmit="return valida_deletar();">
    ...
    <?php
    $busca = $con_cliente->executa("SELECT * FROM vestibulares");
    ...
    <table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
        ...
        <tbody>
                ...
                while($item = mysqli_fetch_assoc($busca)) {
                    ?>
            <tr>
                <td class="campo_checkbox"><label for="checkbox<?=$item['id_vestibulare']?>" id="label<?=$item['id_vestibulare']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_vestibulare']?>" value="<?=$item['id_vestibulare']?>" /></td>
                <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_vestibulare']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
