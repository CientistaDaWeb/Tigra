...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_tg_modulo']?>" id="label<?=$item['id_tg_modulo']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_tg_modulo']?>" value="<?=$item['id_tg_modulo']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_tg_modulo']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['modulo']?></td>
            <td><?=decripfy($item['pasta'],'m0dul0')?></td>
