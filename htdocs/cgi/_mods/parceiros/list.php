...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_parceiro']?>" id="label<?=$item['id_parceiro']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_parceiro']?>" value="<?=$item['id_parceiro']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_parceiro']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['parceiro']?></td>
		</tr>
<?php
	}
