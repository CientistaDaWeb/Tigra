...
if($action) {
    ...
    if($action == 'edit') {
        $query = 'UPDATE obras_fotos SET legenda = "'.$legenda.'" WHERE id_obras_foto = '.$id;
        $con->query($query);
    }

    if($action == 'del') {
        $dominio = decripfy($_SESSION['dominio'],'h0s7');
        $query = 'SELECT imagem FROM obras_fotos WHERE id_obras_foto = '.$id;
        $retorno = $con->query($query);
        if($retorno && $retorno->num_rows > 0) {
            $retorno = $retorno->fetch_assoc();
            unlink('/home/weentigra/www/images/'.$dominio.'/obras/galeria/'.$retorno['imagem']);
            unlink('/home/weentigra/www/images/'.$dominio.'/obras/galeria/thumbs/'.$retorno['imagem']);
        }
    ...
    /*Pega os fotos*/
    $query = 'SELECT * FROM obras_fotos WHERE id_obra = '.$id_obra.' LIMIT '.$inicio.','.$qtd;
    $fotos = $con->query($query);
    if($fotos && $fotos->num_rows > 0) {
        while($foto = $fotos->fetch_assoc()) {
            ?>
<div class="box">
    <img src="http://images.weentigra.com.br/<?=decripfy($_SESSION['dominio'],"h0s7")?>/obras/galeria/thumbs/<?=$foto['imagem']?>" />
