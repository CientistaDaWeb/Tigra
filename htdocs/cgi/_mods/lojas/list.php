...
<?php
$query = 'SELECT r.id_loja, r.nome, e.uf FROM lojas r INNER JOIN estados e ON e.id_estado = r.id_estado';
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_loja']?>" id="label<?=$item['id_loja']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_loja']?>" value="<?=$item['id_loja']?>" /></td>
            <td class="campo_editar"><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_loja']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
            <td><?=$item['uf']?></td>
		</tr>
<?php
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrada nenhum loja.</span></div>
<?php
}
?>
</form>
