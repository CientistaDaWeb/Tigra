...
<?php
$busca = $con_cliente->executa("SELECT * FROM filiais");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_filiai']?>" id="label<?=$item['id_filiai']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_filiai']?>" value="<?=$item['id_filiai']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_filiai']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
		</tr>
<?php
	}
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhuma filial.</span></div>
<?php
}
?>
</form>
