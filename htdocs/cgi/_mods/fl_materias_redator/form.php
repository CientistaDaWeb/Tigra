...
<table id="formulario">
    ...
    <tr>
    	<td class="tit_campo">Redator:</td>
    </tr>
    <tr>
    	<td><select name="id_redatore" class="inpute">
        	<?php
			$redatores = $con_cliente->executa("SELECT * FROM redatores ORDER BY nome");
			if($redatores && mysqli_num_rows($redatores)>0){
				while($redator = mysqli_fetch_assoc($redatores)){
				?>
                <option value="<?=$redator['id_redatore']?>" <?php if($redator['id_redatore'] == $id_redatore){?>selected="selected"<?php } ?>><?=$redator['nome']?></option>
                <?php
				}
			}
			?>        
    ...
    <tr>
    	<td class="campo_radio"><label for="radio2" class="radio_<?=$radio_stat[2]?>">Publicado</label>
...
<table id="table_botoes_rodape">
	<tr>	
		...
		if($id){
		?>
        <td><a href="http://www.<?=decripfy($_SESSION['dominio'],'h0s7')?>/<?=$cat?>/<?=$subcat?>/<?=$id?>/" id="bt_visualizar" target='_blank'>Visualizar Matéria</a></td>
        <?php
        }
        ?>
    </tr>
...
function deleta_audio(id){
	...
	$.ajax({
		type: "POST",
		url: "<?=$url_base?>/cgi/_mods/<?=$tg_mod?>/upload_audios.php?noob="+ new Date().getTime(),
		data: "id_materia=<?=$id_materia?>&del_item="+id,
		success: function(msg){
			$('#audios').empty();
			$("#audios").append(msg);
...
function muda_tag(id_tag){
	...
	$.ajax({
		type: "POST",
		url: "<?=$url_base?>/cgi/_mods/<?=$tg_mod?>/upload_tag.php?noob="+ new Date().getTime(),
...
<div id="div_tag">
    <form id="upload_tag" enctype="multipart/form-data" method="POST">
        ...
        <tr>
            ...
            <td><button onClick="micoxUpload(this.form,'<?=$url_base?>/cgi/_mods/<?=$tg_mod?>/upload_tag.php','tags','<div class=carregando></div>Enviando...','Erro ao Enviar'); zera_sessao(); return false;" type="button">Adicionar</button></td>
        </tr>
        </table>        
        </div>
        </form>
...
<div id="div_video">
    <form id="upload_flash" enctype="multipart/form-data" method="POST">
        ...
        <tr>
            <td><button onClick="micoxUpload(this.form,'<?=$url_base?>/cgi/_mods/<?=$tg_mod?>/upload_videos.php','videos','<div class=carregando></div>Enviando...','Erro ao Enviar'); zera_sessao(); return false;" type="button">Enviar</button></td>
...
<div id="div_audio">
    <form id="upload_flash" enctype="multipart/form-data" method="POST">
        ...
        <tr>
            <td><button onClick="micoxUpload(this.form,'<?=$url_base?>/cgi/_mods/<?=$tg_mod?>/upload_audios.php','audios','<div class=carregando></div>Enviando...','Erro ao Enviar'); zera_sessao(); return false;" type="button">Enviar</button></td>
