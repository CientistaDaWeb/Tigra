...
if($id)	{
	...
	$pesquisa = new redatores();
	$pesquisa->busca($id);
	$foto = $pesquisa->foto;
	$perfil = $pesquisa->perfil;
	$validator = "";
}else{
	?>
...
<table id="formulario">
    ...
    <tr>
		...
		if($foto){
		?>
        <img src="http://www.<?=decripfy($_SESSION['dominio'],"h0s7")?>/_img/redatores/thumbs/<?=$foto?>" /><br />
        <?php
		}
		?>
        <input type="file" name="foto" id="foto" class="inpute">
        </td>
    </tr>
    <tr>
    	<td class="tit_campo">Perfil</td>
    </tr>
    <tr>
        <td><textarea class="inpute" name="perfil" id="perfil"><?=$perfil?></textarea></td>
    </tr>
</table>
