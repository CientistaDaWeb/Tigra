...
<?php
$busca = $con_cliente->executa("SELECT * FROM anuncios, anunciantes WHERE anuncios.id_anunciante = anunciantes.id_anunciante");
...
<table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
    <thead>    	
     ...
     <tbody>
<?php
	$tamanho = array(1=>'Meio', 'Esquerda', 'Direita');
	while($item = mysqli_fetch_assoc($busca)){
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_anuncio']?>" id="label<?=$item['id_anuncio']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_anuncio']?>" value="<?=$item['id_anuncio']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_anuncio']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['anunciante']?></td>
            <td><?=$tamanho[$item['tamanho']]?></td>
            <td><?=number_format((($item['impressoes_efetuadas']/$item['impressoes_contratadas'])*100),2,".",",")?></td>
		</tr>
<?php
	}
?>
