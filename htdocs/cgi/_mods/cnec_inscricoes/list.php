...
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" id="form_deletar" onsubmit="return valida_deletar();">
    ...
    <?php
    $busca = $con_cliente->executa('SELECT * FROM vestibulares_inscricoes AS vi, vestibulares_boletos AS vb WHERE vi.id_vestibulares_inscricoe = vb.id_vestibulares_inscricoe');
    ...
    <table id="lista" class="display" border="0" cellpadding="0" cellspacing="0">
        ...
        <tbody>
                ...
                while($item = mysqli_fetch_assoc($busca)) {
                    ?>
            <tr>
                <td class="campo_checkbox"><label for="checkbox<?=$item['id_vestibulares_inscricoe']?>" id="label<?=$item['id_vestibulares_inscricoe']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_vestibulares_inscricoe']?>" value="<?=$item['id_vestibulares_inscricoe']?>" /></td>
                <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_vestibulares_inscricoe']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
        ...
        ?>
    <div><span class="vazio">Não foi encontrada nenhuma inscrição pendente.</span></div>
        <?php
    }
    ?>
</form>
