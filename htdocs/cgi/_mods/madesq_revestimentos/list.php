...
<?php
$busca = $con_cliente->executa('SELECT * FROM revestimentos');
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_revestimento']?>" id="label<?=$item['id_revestimento']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_revestimento']?>" value="<?=$item['id_revestimento']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_revestimento']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
