...
<?php
$busca = $con_cliente->executa("SELECT * FROM materias_subcategorias, materias_categorias WHERE materias_subcategorias.id_materias_categoria = materias_categorias.id_materias_categoria");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_materias_subcategoria']?>" id="label<?=$item['id_materias_subcategoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_materias_subcategoria']?>" value="<?=$item['id_materias_subcategoria']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_materias_subcategoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhuma subcategoria de mat&eacute;ria.</span></div>
<?php
}
?>
</form>
