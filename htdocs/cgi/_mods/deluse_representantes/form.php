...
if ($id) {
    ...
    $fones = $pesquisa->fones;
    $emails = $pesquisa->emails;
}
?>
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" enctype="multipart/form-data" id="form_edicao" onsubmit="return ween_validator()">
    ...
    <table id="formulario">
        ...
        <tr>
            <td>
                <input id="contato" name="contato" class="inpute medio obrigatorio" value="<?=$contato?>" />
            </td>
        </tr>
        <tr>
            <td class="tit_campo">
        ...
        <tr>
            <td>
                <input id="cidade_regiao" name="cidade_regiao" class="inpute gde obrigatorio" value="<?=$cidade_regiao?>" />
            </td>
        </tr>
        <tr>
            <td class="tit_campo">
        ...
        <tr>
            <td>
                <select name="id_estado" id="id_estado" class="inpute">
                    <?php 
        ...
        <tr>
            <td>
                <textarea id="fones" name="fones"><?=$fones?></textarea>
            </td>
        </tr>
        <tr>
            <td class="tit_campo">
        ...
        <tr>
            <td>
                <textarea id="emails" name="emails"><?=$emails?></textarea>
            </td>
        </tr>
    </table>
    <?php 
    ...
    <div class="subtitulos" id="tit_estado">
        Estados de Atuação
    </div>
    <div id="status_estado" class="sucesso">
    </div>
    <div>
        <?php 
        $checkbox_stat = array();
        ...
        if ($estados && mysqli_num_rows($estados) > 0) {
            while ($estadiu = mysqli_fetch_assoc($estados)) {
                if ($id_estado == $estadiu['id_estado'])
                    $selected = 'selected="selected"';
                
        ?>
        <div class="quatrocolunas">
            <label for="permit<?=$estadiu['id_estado']?>" onclick="muda_estado(<?=$estadiu['id_estado']?>)">
<?= utf8_encode($estadiu['estado'])?>
            </label>
