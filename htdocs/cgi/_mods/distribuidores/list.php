...
<?php
$query = 'SELECT r.id_distribuidore, r.nome, e.uf FROM distribuidores r INNER JOIN estados e ON e.id_estado = r.id_estado';
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_distribuidore']?>" id="label<?=$item['id_distribuidore']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_distribuidore']?>" value="<?=$item['id_distribuidore']?>" /></td>
            <td class="campo_editar"><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_distribuidore']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['nome']?></td>
            <td><?=$item['uf']?></td>
		</tr>
<?php
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhum distribuidor.</span></div>
<?php
}
?>
</form>
