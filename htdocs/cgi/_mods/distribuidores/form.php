...
if($id) {
    $pesquisa = new distribuidores();
    $pesquisa->busca($id);
    $id_distribuidore = $pesquisa->id_distribuidore;
    $nome = $pesquisa->nome;
...
<form action="<?=$url_base?>/cgi/<?=$mod?>/action" method="post" enctype="multipart/form-data" id="form_edicao" onsubmit="return ween_validator()">
    <input type="hidden" value="<?=$id_distribuidore?>" name="id_distribuidore" id="id_distribuidore" />
    <table id="formulario">
        <tr>
    ...
    <script type="text/javascript">
        function muda_estado(id_estado){
            ...
            $.ajax({
                type: "POST",
                url: "<?=$url_base?>/cgi/_mods/distribuidores/upload_estado.php?noob="+ new Date().getTime(),
                data: "id_distribuidore=<?=$id_distribuidore?>&id_estado="+escape(id_estado)+"&acao="+escape(acao),
                beforeSend: function(){
    ...
    <div>
            <?php
            $checkbox_stat = array();
            $permissoes = $con_cliente->executa("SELECT * FROM distribuidores_estados WHERE id_distribuidore = $id_distribuidore");
