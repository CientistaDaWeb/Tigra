...
<?php
$busca = $con_cliente->executa("SELECT * FROM obras_categorias");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_obras_categoria']?>" id="label<?=$item['id_obras_categoria']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_obras_categoria']?>" value="<?=$item['id_obras_categoria']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_obras_categoria']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">N&atilde;o foi encontrada nenhuma categoria de obra.</span></div>
<?php
}
?>
</form>
