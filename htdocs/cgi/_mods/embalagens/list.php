...
<?php
$query = 'SELECT * FROM embalagens ORDER BY embalagem_pt';
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><label for="checkbox<?=$item['id_embalagen']?>" id="label<?=$item['id_embalagen']?>"></label><input type="checkbox" class="crirHiddenJS" name="del_item[]" id="checkbox<?=$item['id_embalagen']?>" value="<?=$item['id_embalagen']?>" /></td>
            <td class="campo_editar"><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_embalagen']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
            <td><?=$item['embalagem_pt']?></td>
            <td><?=$item['embalagem_es']?></td>
			<td><?=$item['embalagem_en']?></td>
		</tr>
<?php
	}
?>
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrada nenhuma embalagem.</span></div>
<?php
}
?>
</form>
