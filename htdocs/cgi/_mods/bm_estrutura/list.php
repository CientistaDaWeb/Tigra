...
<?php
$busca = $con_cliente->executa("SELECT id_estrutura, descricao FROM estruturas");
...
<?php
	...
	?>
		<tr>
        	<td class="campo_checkbox"><input type="checkbox" name="del_item[]" id="checkbox<?=$item['id_estrutura']?>" value="<?=$item['id_estrutura']?>" /></td>
            <td><a href="<?=$url_base?>/cgi/<?=$mod?>/form/<?=$item['id_estrutura']?>"><img src="_css/_img/btn/btn_editar.png" /></a></td>
...
<?php
	...
	?>
		<div><span class="vazio">Não foi encontrado nenhum índice.</span></div>
<?php
}
?>
</form>
