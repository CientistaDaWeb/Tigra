...
class upload {
	...
	public function img_resize($img_status = false, $tmb_status = false){
		...
		if($this->stats_move_file){ #verifica se o arquivo foi movido com sucesso
			...
			break;
			case 'image/jpeg' 	:
				$this->img_org = imagecreatefromjpeg($this->file_full_path);
				break;
	...
	*/
    ...
    $imgOriginal = imagecreatefromjpeg($this->file_tmp_dir.$this->ftp_id.'/'.$this->file_name);
    list($imagemLarguraOriginal, $imagemAlturaOriginal) = getimagesize($this->file_tmp_dir.$this->ftp_id.'/'.$this->file_name);
	$radio = ($imagemAlturaOriginal / $imagemLarguraOriginal);


    if ($imagemAlturaOriginal < $imagemLarguraOriginal)
		{
			// largura MAIOR que altura
			//$alturaImgCustomizada 		= $imagemLargura * $radio;
			//$larguraImgCustomizada 		= $imagemAltura;
	    	//
			$alturaThumbCustomizada 	= $this->tmb_height;
			$larguraThumbCustomizada	= $this->tmb_height / $radio;
		}
		else if($imagemAlturaOriginal > $imagemLarguraOriginal)
		{
			// altura MAIOR que largura
			//$alturaImgCustomizada 		= $imagemAltura;
			//$larguraImgCustomizada 		= $imagemAltura / $radio;
			//
			$alturaThumbCustomizada 	= $this->tmb_width * $radio;
			$larguraThumbCustomizada	= $this->tmb_width;
		}
		else
		{
        ...
        $imgThumbCustomizada = imagecreatetruecolor($larguraThumbCustomizada, $alturaThumbCustomizada);
			imagecopyresampled
			(
				$imgThumbCustomizada,
				$imgOriginal, 0, 0, 0, 0,
				$larguraThumbCustomizada,
				$alturaThumbCustomizada,
				$imagemLarguraOriginal,
				$imagemAlturaOriginal
			);


            $imgThumb = imagecreatetruecolor($this->tmb_width, $this->tmb_height);


            $new_w = (($larguraThumbCustomizada / 2) - ($this->tmb_width / 2));
    $new_h = (($alturaThumbCustomizada / 2) - ($this->tmb_height / 2));

			imagecopyresampled
			(
			$imgThumb,
			$imgThumbCustomizada, 0, 0, $new_w, $new_h,
			$larguraThumbCustomizada,
			$alturaThumbCustomizada,
			$larguraThumbCustomizada,
			$alturaThumbCustomizada
        ...
        imagejpeg($imgThumb, $this->file_tmp_dir.$this->ftp_id.'/tmb/'.$this->img_db_name, 85);
